...
* Make sure you add the Go/bin folder (`$HOME/go/bin`) to the system path (Run: `export PATH=$PATH:$HOME/go/bin` on linux) ([^Execute this](didact://?commandId=vscode.didact.sendNamedTerminalAString&text=RHIOperatorTerminal$$export%20PATH=$PATH:$HOME/go/bin "Call `export PATH=$PATH:$HOME/go/bin` in the terminal window called 'RHIOperatorTerminal' to add the Go/bin folder to the system path"){.didact}))
